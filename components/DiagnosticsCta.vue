<template>
  <section class="cta-diagnostics">
    <div class="cta-diagnostics__overlay">
      <h2 class="cta-diagnostics__title">ПОЛУЧИТЕ ДИАГНОСТИКУ БЕСПЛАТНО</h2>

      <form class="cta-diagnostics__form" @submit.prevent="submit">
        <input
          v-model="form.name"
          type="text"
          class="input"
  ...
  try {
    await $fetch('/api/send-lead', {
      ...
      body: {
        ...
        source: 'cta-diagnostics'
      }
    })
    // redirect to complete page for conversion tracking
    navigateTo('/complete?source=cta-diagnostics')
  } catch (e) {
    console.error(e)
